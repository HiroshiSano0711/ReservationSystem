module Reservations
  module TimeResolver
    ...
    # フォーマットは以下の正規表現で統一（ISO8601が広すぎるので明示的に定義する）
    #   年月日、T、時分秒までは必須。
    #   末尾のオフセット（Z または +09:00 / -0500 等）は任意
    #   オフセットがない場合はそのユーザーのタイムゾーンで変換する。
    #
    #   使用できるフォーマット
    #     例1： "2026-01-01T10:00:00"
    #     例2： "2026-01-01T10:00:00Z"
    #     例3： "2026-01-01T10:00:00+0900"
    #     例4： "2026-01-01T10:00:00-0400"
    # なぜこのように変換するか？は、入力はオフセットがない場合（CSV、日時だけ選択するUIなど）もあるためです。
    # グローバルスタンダード、国際対応といった理由はどちらかというと薄いです。
    # 対応する場合でも、time_zoneの引数を指定するだけで良い。
